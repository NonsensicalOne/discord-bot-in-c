...
void read_websocket_buffer(int valread, char *buffer) {
    ...
    if ((u_buf[0] & 0x0F) == 0x01) {
        ...
        int data_len = (payload_len == 126) ? ((u_buf[2] << 8) | u_buf[3]) : payload_len;
...
void send_https_message(struct s2n_config *cfg, const char *chan, const char *text, const char *token) {
    ...
    sprintf(req, "POST /api/v10/channels/%s/messages HTTP/1.1\r\n"
                 "Host: discord.com\r\n"
                 "Authorization: Bot %s\r\n"
...
int main(void) {
  ...
  return -1;
    ...
    send_websocket_text(conn, "{\"op\":1,\"d\":null}"); // first heartbeat that is needed 
    
    char *token = getenv("DISCORD_TOKEN");
    char identify[1024];
    sprintf(identify, "{\"op\":2,\"d\":{\"token\":\"%s\",\"intents\":33280,"
                      "\"properties\":{\"os\":\"linux\",\"browser\":\"c\",\"device\":\"c\"}}}", token);
